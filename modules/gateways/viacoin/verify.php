...
function verify_payment($payment_id, $amount, $amount_via, $invoice_id, $fee, $status, $gatewaymodule, $hash, $secretKey, $currency){
	...
	if ($payment_id !="") {
		...
		if ($hash != md5($invoice_id . $payment_id . $amount_via . $secretKey)) {
		...
		// send each viacoin tx in the mempool to handle_whmcs
		if ($check_mempool) {
			$get_payments_method = $viacoin_daemon->get_transfers('pool', true);
			foreach ($get_payments_method["pool"] as $tx => $transactions) {
				...
				if(isset($txn_amt)) { 
					return handle_whmcs($invoice_id, $amount_via, $txn_amt, $txn_txid, $txn_payment_id, $payment_id, $currency, $gatewaymodule);
				}
			}
		}
		// send each viacoin tx to handle_whmcs
		$get_payments_method = $viacoin_daemon->get_payments($payment_id);
...
function handle_whmcs($invoice_id, $amount_via, $txn_amt, $txn_txid, $txn_payment_id, $payment_id, $currency, $gatewaymodule) {
	$amount_atomic_units = $amount_via * 1000000000000;
	
	//check if viacoin tx already exists in whmcs 
	...
	if ($txn_payment_id == $payment_id) {
		if (!$transaction_exists) {
			...
			$fiat_paid = via_to_fiat($txn_amt, $currency);
		...
		if ($txn_amt * 1.02 >= $amount_atomic_units) {
			...
			return "Error: Amount " . $txn_amt / 1000000000000 . " VIA too small. Please send full amount or contact customer service. Transaction ID: " . $txn_txid . ". Payment ID: " . $payment_id;
		}
	}
}


function add_payment($command, $invoice_id, $txn_txid, $gatewaymodule, $fiat_paid, $amount_via, $payment_id, $fee) {
	$postData = array(
		'action' => $command,
...
$vefiry = verify_payment($payment_id, $amount, $amount_via, $invoice_id, $fee, $status, $gatewaymodule, $hash, $secretKey, $currency);
echo $vefiry;
